/* Page Corrigés : thème, menu, suivi « fait », recherche, filtres (TD, type), ouverture par l'adresse (#ex5-4) */
(function () {
  "use strict";
  var LS_THEME = "atp_theme", LS_FAITS = "atp_corriges";
  ...
  var choixType = document.getElementById("cxType");
  var cat = "all", type = "all";
  var DIACR = new RegExp("[" + String.fromCharCode(0x300) + "-" + String.fromCharCode(0x36f) + "]", "g");
  function normaliser(s) {
    return (s || "").toLowerCase().normalize("NFD").replace(DIACR, "");
  }
  function filtrer() {
    ...
    sections.forEach(function (sec) {
      ...
      sec.querySelectorAll("article.cx[id]").forEach(function (it) {
        var okType = type === "all" || !!it.querySelector(".cx-tag." + type);
        var montre = okCat && okType && (!q || normaliser(it.textContent).indexOf(q) !== -1);
        it.hidden = !montre;
        if (montre) vus++;
      });
      sec.hidden = !okCat || vus === 0;
    });
  ...
  if (choixType) choixType.addEventListener("change", function () { type = choixType.value; filtrer(); });
  ...
  function ouvrirAncre() {
    ...
    if (el.hidden || (el.closest("section") && el.closest("section").hidden)) {
      ...
      if (choixType) choixType.value = "all";
      filtrer();
    }
